...
export const NavBar = ({ startFull }: Props) => {
    const { toggle, isDarkMode } = useDarkModeEX();
    const [fullMode, setFullMode] = useState(startFull);
    ...
    return (
        <>
            <nav
                className={`fixed ${
                    fullMode ? 'w-52' : 'w-16'
                } h-screen dark:bg-gray-900 bg-gray-200 text-kanata-blue dark:text-kanata-gold transition-all duration-150 ease-linear`}
            >
                <ul
